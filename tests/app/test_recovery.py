...
from curator.core.recovery import SENTINEL_KEY, recover_queued_uploads


@pytest.mark.asyncio
async def test_skips_recovery_when_sentinel_key_present(mocker):
    mock_redis = mocker.patch("curator.core.recovery.redis_client")
    mock_redis.set.return_value = None
    mock_enqueue = mocker.patch(
        "curator.core.recovery.enqueue_uploads", new_callable=AsyncMock
    )
...
async def test_sets_sentinel_key_when_no_queued_uploads(mocker):
    ...
    mocker.patch("curator.core.recovery.get_session", MagicMock())
    mocker.patch(
        "curator.core.recovery.get_queued_uploads_for_recovery", return_value=[]
    )
    mocker.patch("curator.core.recovery.enqueue_uploads", new_callable=AsyncMock)
...
async def test_reenqueues_queued_uploads_after_redis_restart(mocker):
    ...
    mocker.patch("curator.core.recovery.get_session", MagicMock())
    mocker.patch(
        "curator.core.recovery.get_queued_uploads_for_recovery", return_value=queued
    )
    mocker.patch("curator.core.recovery.decrypt_access_token", return_value=mock_token)
    mock_client = MagicMock()
    mock_client.get_user_groups.return_value = set()
    mocker.patch("curator.core.recovery.MediaWikiClient", return_value=mock_client)
    mock_enqueue = mocker.patch(
        "curator.core.recovery.enqueue_uploads", new_callable=AsyncMock
    )
...
async def test_groups_uploads_by_userid_and_batch(mocker):
    ...
    mocker.patch(
        "curator.core.recovery.decrypt_access_token",
...
async def test_marks_uploads_failed_when_token_is_corrupted(mocker):
    ...
    mock_fail = mocker.patch("curator.core.recovery.mark_uploads_expired")
    mock_enqueue = mocker.patch(
        "curator.core.recovery.enqueue_uploads", new_callable=AsyncMock
    )
...
async def test_marks_all_expired_uploads_in_single_session_call(mocker):
    ...
    mocker.patch(
        "curator.core.recovery.decrypt_access_token", side_effect=Exception("invalid")
    )
    mock_fail = mocker.patch("curator.core.recovery.mark_uploads_expired")
    mocker.patch("curator.core.recovery.enqueue_uploads", new_callable=AsyncMock)
