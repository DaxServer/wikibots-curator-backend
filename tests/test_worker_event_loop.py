...
def test_process_one_runs_without_event_loop_closed(runs):
    ...
    with (
        ...
        patch.object(ingest, "count_open_uploads_for_batch") as mock_count,
        ...
        mock_get.return_value = ingest.UploadRequest(
            id=1,
            batchid=1,
            userid="user",
            status="queued",
        ...
        for _ in range(runs):
            assert ingest.process_one(1, "input", "enc", "user") is True
