...
async def test_admin_get_batches_success(mock_session):
    with (
        ...
        result = await admin_get_batches(page=1, limit=100, session=mock_session)
...
async def test_admin_get_users_success(mock_session):
    with (
        ...
        result = await admin_get_users(page=1, limit=100, session=mock_session)
...
async def test_admin_get_upload_requests_success(mock_session):
    with (
        ...
        result = await admin_get_upload_requests(
            page=1, limit=100, session=mock_session
        )
