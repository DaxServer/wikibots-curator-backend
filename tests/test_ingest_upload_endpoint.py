...
from curator.ingest import ingest_upload
from curator.ingest import UploadItemsPayload
...
def test_ingest_upload_enqueues_with_integer_ids():
    ...
    payload = UploadItemsPayload(
        handler="mapillary",
        items=[UploadItem(id="img1", input="seq1", title="T1", wikitext="W1")],
    )
    ...
    with (
        patch("curator.ingest.create_upload_request", return_value=[upload_req]),
        patch("curator.ingest.ingest_process_one.delay") as mock_delay,
    ):
