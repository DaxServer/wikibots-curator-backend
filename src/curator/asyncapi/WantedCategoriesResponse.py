# Auto-generated by @asyncapi/modelina
# DO NOT EDIT MANUALLY

from __future__ import annotations

from typing import Literal

from pydantic import BaseModel, ConfigDict, Field

from .WantedCategoriesResponseData import WantedCategoriesResponseData


class WantedCategoriesResponse(BaseModel):
    type: Literal["WANTED_CATEGORIES_RESPONSE"] = Field(
        default="WANTED_CATEGORIES_RESPONSE", frozen=True
    )
    data: WantedCategoriesResponseData = Field()
    nonce: str = Field(description="""Timestamp nonce to ensure change detection""")

    model_config = ConfigDict(populate_by_name=True)
