...
class JobConfig(BaseModel):
    ...
    continuous: Optional[bool] = Field(False, description="If a job should be always running.")
    ...
    filelog: Optional[bool] = Field(False, description="Whether this job uses filelog or not.")
    filelog_stderr: Optional[str] = Field(None, description="Path to the stderr file log.")
    filelog_stdout: Optional[str] = Field(None, description="Path to the stdout file log.")
    memory: Optional[str] = Field(None, description="Job memory resource limit.")
    mount: Optional[str] = Field(None, description="NFS mount configuration for the job.")
    port: Optional[int] = Field(None, description="Port to expose for the job. Applicable only when continuous is true.")
    replicas: Optional[int] = Field(None, description="Number of replicas to be used for the job. Configurable only when continuous is true.")
    retry: Optional[int] = Field(0, description="Job retry policy. Zero means don't retry at all (the default)")
    schedule: Optional[str] = Field(None, description="If the job is a cronjob, execution schedule.")
