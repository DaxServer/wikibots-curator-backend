...
- **Per-chunk retryable errors** (substring match on `error.code`): `UploadStashFileException`, `uploadstash-exception` (bare variant from `handleStashException` default case — also covers `UploadChunkFileException` falling through), `UploadChunkFileException`, `JobQueueError` — transient infrastructure errors that retry up to 4 attempts with 3/5/10s delays
