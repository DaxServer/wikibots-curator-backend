...
- **Per-chunk retryable errors**: substring match on `error.code` for `UploadStashFileException`, `uploadstash-exception` (bare variant from `handleStashException` default case — also covers `UploadChunkFileException` falling through), `UploadChunkFileException`, `JobQueueError`, `internal_api_error_` (prefix, covers `DBQueryError` and similar transient MW DB errors); also matches `backend-fail-internal` in `error.info` (handles `stashfailed` code with Swift storage errors) — transient infrastructure errors that retry up to 4 attempts with 3/5/10s delays
- **Final commit retryable errors** (substring match on `error.code`): `backend-fail-internal`, `JobQueueError`, `internal_api_error_` (prefix) — retried with the same 4-attempt / 3/5/10s logic. The final commit has its own separate retry loop in `upload_file()`.
...
- **`stashfailed: Chunked upload is already completed`** is handled in `upload_file()` (not `_upload_chunk`) — when a chunk retry gets this error and a valid `file_key` exists, the chunk loop breaks and proceeds to the final commit; the stash was already complete despite the transient error on the prior attempt
...
- For complex mock behavior, use module-level helper functions (prefix with `_`) passed to `side_effect`; for stateful mocks (e.g. call-count tracking), pass a mutable dict via `functools.partial(helper, state_dict)`
