...
- **Final commit retryable errors** (substring match on `error.code`): `backend-fail-internal` — storage backend transient error (T420956), retried with the same 4-attempt / 3/5/10s logic. The final commit has its own separate retry loop in `upload_file()`.
- **Duplicate detection**: Duplicate warnings appear on the final chunk response during stash phase (with `stash=1`), NOT during final commit. Two warning types are handled: `warnings.duplicate` (list of titles with same content) raises `DuplicateUploadError` immediately. `warnings.exists` (string — a single title with same filename) fetches the remote file's SHA1 via `get_file_sha1()` and raises `DuplicateUploadError` only if hashes match; otherwise falls through to a generic warning failure. `upload_file()` accepts `file_sha1` to enable this comparison; `upload_file_chunked()` in `commons.py` passes the hash computed during download.
